...
class NextEventPredictor():
    ...
    def _predict_next_event_shared_cat_next(self, parameters, vectorizer):
        ...
        #prediction of prediction
        if 'process_multi_pred_ss' not in st.session_state:
            st.session_state['process_multi_pred_ss'] = dict()
            for _zx in range(parameters['multiprednum']):
                st.session_state['process_multi_pred_ss']["pm_multipredict{0}".format(_zx + 1)] = {'ac_pred': [], 'ac_prob': [],
                                                                                          'rl_pred': [], 'rl_prob': [],
                                                                                          'tm_pred': []}

        if 'pos_tm_ss' not in st.session_state:
            st.session_state['pos_tm_ss'] = [[0]]

        if 'process_tm_ss' not in st.session_state:
            st.session_state['process_tm_ss'] = []
        ...
        if 'initial_process_prediction' not in st.session_state:
            st.session_state['initial_process_prediction'] = dict()
            for _lx in range(parameters['multiprednum']):
                st.session_state['initial_process_prediction']["pr_initpredict{0}".format(_lx + 1)] = {'pr_ac_ss': [],
                                                                                               'pr_rl_ss': []}
        ...
        for i, _ in enumerate(self.spl['prefixes']['activities'][:pred_fltr_idx]):
            ...
            if (i == pred_fltr_idx - 1) and (len(self.spl['prefixes']['activities']) != pred_fltr_idx):
                ...
                _time_hist = self.spl['prefixes']['times'][:pred_fltr_idx][i]

                _time_hist = np.concatenate((_time_hist, np.array([[predictions[2]]])))
                series_time_hist = [np.array(_time_hist[:idx]) for idx in range(1, pred_fltr_idx + 2)]  # time
                ...
                for _ik in range(parameters['multiprednum']):
                    ...
                    if  parameters['multiprednum'] > 1:
                        ...
                        # ---process logic
                        _activity_hist.append(float(predictions[0][_ik]))
                        _role_hist.append(float(predictions[1][_ik]))
                        ...
                        # ---process logic
                        _activity_hist.append(float(predictions[0]))
                        _role_hist.append(float(predictions[1]))
                    ...
                    if 'initial_process_prediction' in st.session_state:
                        st.session_state['initial_process_prediction']['pr_initpredict' + str(_ik + 1)]['pr_ac_ss'].clear()
                        st.session_state['initial_process_prediction']['pr_initpredict' + str(_ik + 1)]['pr_rl_ss'].clear()
                        st.session_state['initial_process_prediction']['pr_initpredict' + str(_ik + 1)]['pr_ac_ss'].extend(_activity_hist)
                        st.session_state['initial_process_prediction']['pr_initpredict' + str(_ik + 1)]['pr_rl_ss'].extend(_role_hist)

                        series_activity_hist = [_activity_hist[:idx] for idx in range(1, pred_fltr_idx + 2)]  # activity
                        series_role_hist = [_role_hist[:idx] for idx in range(1, pred_fltr_idx + 2)]  # role

                        if 'multi_pred_ss' in st.session_state:

                            self._predict_next_event_process(parameters, vectorizer, series_activity_hist, series_role_hist, series_time_hist, _ik)

                    del _activity_hist[-1]
                    del _role_hist[-1]
                # -Time
                st.session_state['pos_tm_ss'].extend([[abs(preds[2][0][0])]])
                st.session_state['process_tm_ss'].clear()
                st.session_state['process_tm_ss'].extend(_time_hist.tolist())
                _time_hist = _time_hist[:-1]
                ...
                if 'initial_prediction' in st.session_state:
                    ...
                    for _ih in range(parameters['multiprednum']):
                        ...
                        #----Check the Vector length is same or not
                            if (len(self.spl['prefixes']['activities'][:pred_fltr_idx+1]) == len(serie_predict_ac)) and (
                                    len(self.spl['prefixes']['roles'][:pred_fltr_idx+1]) == len(serie_predict_rl)) and (
                                    len(self.spl['prefixes']['times'][:pred_fltr_idx+1]) == len(serie_predict_tm)) and (
                                    'multi_pred_ss' in st.session_state):

                                # print("--------------Generative History", (_ih + 1), "--------------------")
            ...
            # print("st.session_state after 2 : ", st.session_state)
    ...
    def _predict_next_event_shared_cat_pred(self, parameters, vectorizer, serie_predict_ac, serie_predict_rl, serie_predict_tm, index):
        ...
        #----------------------------------------------
        # print("ac series : ", serie_predict_ac[-1])
        # print("rl series : ", serie_predict_rl[-1])
        # print("tm series : ", serie_predict_tm[-1])
        #
        # x_ac_ngram = (np.append(
        #         np.zeros(parameters['dim']['time_dim']),
        #         np.array(serie_predict_ac[-1]),
        #         axis=0)[-parameters['dim']['time_dim']:]
        #     .reshape((1, parameters['dim']['time_dim'])))
        # #----------------------------------------------------------------------------------
        # x_rl_ngram = (np.append(
        #         np.zeros(parameters['dim']['time_dim']),
        #         np.array(serie_predict_rl[-1]),
        #         axis=0)[-parameters['dim']['time_dim']:]
        #     .reshape((1, parameters['dim']['time_dim'])))
        # # ----------------------------------------------------------------------------------
        # # ----------------------------------------------------------------------------------
        # # times input shape(1,5,1)
        # times_attr_num = (serie_predict_tm[-1].shape[1])
        # x_t_ngram = np.array(
        #     [np.append(np.zeros(
        #         (parameters['dim']['time_dim'], times_attr_num)),
        #         serie_predict_tm[-1], axis=0)
        #         [-parameters['dim']['time_dim']:]
        #         .reshape((parameters['dim']['time_dim'], times_attr_num))])
        # # add intercase features if necessary
        # # ----------------------------------------------------------------------------------
        ...
        #     inputs = [x_ac_ngram, x_rl_ngram, x_t_ngram]
        # elif vectorizer in ['inter']:
        #     inter_attr_num = (self.spl['prefixes']['inter_attr'][:pred_fltr_idx][pred_fltr_idx-1].shape[1])
        #     x_inter_ngram = np.array(
        #         [np.append(np.zeros((
        #             parameters['dim']['time_dim'], inter_attr_num)),
        #             self.spl['prefixes']['inter_attr'][:pred_fltr_idx][pred_fltr_idx-1], axis=0)
        #             [-parameters['dim']['time_dim']:]
        #             .reshape((parameters['dim']['time_dim'], inter_attr_num))])
        #     inputs = [x_ac_ngram, x_rl_ngram, x_t_ngram, x_inter_ngram]
        # # predict
    ...
    def _predict_next_event_process(self, parameters, vectorizer, hist_predict_ac, hist_predict_rl, hist_predict_tm, index):
        """Generate business process suffixes using a keras trained model.
        Args:
            model (keras model): keras trained model.
            prefixes (list): list of prefixes.
        ...
        # for i, _ in enumerate(hist_predict_ac):

        # print("how the input looks like : ", hist_predict_ac[i])
        # print("actual input : ", self.spl['prefixes']['activities'])

        # print("----Process Hist------")
        # print("Activity : ", hist_predict_ac)
        # print("Role : ", hist_predict_rl)
        # print("Time : ", hist_predict_tm)

        for i, _ in enumerate(hist_predict_ac):
            x_ac_ngram = (np.append(
                    np.zeros(parameters['dim']['time_dim']),
                    np.array(hist_predict_ac[i]),
                    axis=0)[-parameters['dim']['time_dim']:]
                .reshape((1, parameters['dim']['time_dim'])))
            #----------------------------------------------------------------------------------
            x_rl_ngram = (np.append(
                    np.zeros(parameters['dim']['time_dim']),
                    np.array(hist_predict_rl[i]),
                    axis=0)[-parameters['dim']['time_dim']:]
                .reshape((1, parameters['dim']['time_dim'])))
            # ----------------------------------------------------------------------------------
            # times input shape(1,5,1)
            times_attr_num = (hist_predict_tm[i].shape[1])
            x_t_ngram = np.array(
                [np.append(np.zeros(
                    (parameters['dim']['time_dim'], times_attr_num)),
                    hist_predict_tm[i], axis=0)
                    [-parameters['dim']['time_dim']:]
                    .reshape((parameters['dim']['time_dim'], times_attr_num))])
            # add intercase features if necessary
            # ----------------------------------------------------------------------------------
            ...
            #     inputs = [x_ac_ngram, x_rl_ngram, x_t_ngram]
            # elif vectorizer in ['inter']:
            #     inter_attr_num = (hist_predict_inter.shape[1])
            #     x_inter_ngram = np.array(
            #         [np.append(np.zeros((
            #             parameters['dim']['time_dim'], inter_attr_num)),
            #             hist_predict_inter, axis=0)
            #             [-parameters['dim']['time_dim']:]
            #             .reshape((parameters['dim']['time_dim'], inter_attr_num))])
            #     inputs = [x_ac_ngram, x_rl_ngram, x_t_ngram, x_inter_ngram]
            ...
            if i == pred_fltr_idx:
                ...
                st.session_state['process_multi_pred_ss']["pm_multipredict" + str(index + 1)]['ac_pred'].extend([pos])
                st.session_state['process_multi_pred_ss']["pm_multipredict" + str(index + 1)]['ac_prob'].extend([pos_prob])

                st.session_state['process_multi_pred_ss']["pm_multipredict" + str(index + 1)]['rl_pred'].extend([pos1])
                st.session_state['process_multi_pred_ss']["pm_multipredict" + str(index + 1)]['rl_prob'].extend([pos1_prob])

                st.session_state['process_multi_pred_ss']["pm_multipredict" + str(index + 1)]['tm_pred'].extend(
                    [[abs(preds[2][0][0])]])
